...
%         csvwrite('F:\shiyan\interpretability\OPIMC_GBM1_X.csv',tianx);
        ttime = toc(t1); 
        result_LatLRR = ClusteringMeasure(label, label_total(pass,:)');
        ac = result_LatLRR(1);
        nmi_value = result_LatLRR(5);
        purity    = result_LatLRR(4);
        spe = result_LatLRR(2);
        sen = result_LatLRR(3); 
        fscore= result_LatLRR(6);
%         auc =result_LatLRR(4);
        average_nmi(pass, 1) = nmi_value;
        average_ac(pass, 1) = ac;
        average_pur(pass, 1) = purity;
        average_fscore(pass, 1)=fscore;
        average_spe(pass, 1)=spe;
        average_sen(pass, 1)=sen;
%         average_auc(pass, 1)=auc;
%         [ARi(pass, 1),~,~,~] = RandIndex(label, label_total(pass,:)');   
    end
    Clu_result.ACC    = average_ac(pass, 1);
    Clu_result.SPE=average_spe(pass, 1);
    Clu_result.SEN=average_sen(pass, 1);
    Clu_result.Purity = average_pur(pass, 1);
    Clu_result.NMI    = average_nmi(pass, 1);
    Clu_result.Fscore=average_fscore(pass, 1);
    Clu_result.Bacc=(Clu_result.SPE+Clu_result.SEN)/2;
%     Clu_result.ARi    = ARi(pass,1);  
%     Clu_result.AUC    = average_auc(pass, 1);
