...
% 
% load('F:\incomplete modle\1_Adaptive Weighted Graph Fusion Incomplete\multi-view-datasets-master\random remove\LGG 1p19g1_mxn1.mat')
% load('F:\incomplete modle\1_Adaptive Weighted Graph Fusion Incomplete\multi-view-datasets-master\random remove\LGG 1p19g1_mxn_percent0.mat')
% data=X;
% miss0=folds;
% save('LGG 1p19g1_mxn2.mat','data','miss0','truth');
...
%% Example Title
datadir = 'F:\incomplete modle\1_Adaptive Weighted Graph Fusion Incomplete\multi-view-datasets-master\random remove\';
dataname = {'509mxn2','ye_tu_mxn2','GBM_transfer_mxn2','HGG_MS_BT_mxn2', 'brats_2019_1_mxn2','brats_2019_2_mxn2','REMBRANDT_mxn2'}; 
...
for datai = 1:datanum
    ...
    for per_in = misspre  % per incomplete ratio 5
%         in_ratio = per_in*10;
%         percent = ; %percent = cell(1,10);
        in_ratio = per_in;
        percent =['miss',num2str(in_ratio)] ;
            ...
            for i=1:length(lambda1)
                for j = 1:length(lambda2) 
                    for d = 1:length(lambda3)
                        ...
                        Final_results(2) = mean(metric(:,2));% Spe
                        Final_results(3) = mean(metric(:,3));%Sen
                        Final_results(4) = mean(metric(:,4));%purity
                        Final_results(5) = mean(metric(:,5));%NMI
                        Final_results(6) = mean(metric(:,6));%Fscore
                        Final_results(7) = (Final_results(3)+Final_results(2))/2;
