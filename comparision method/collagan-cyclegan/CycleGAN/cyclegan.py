...
for epoch in range(epochs):
  ...
  for ii,(imgs) in tqdm(enumerate(data_loader),total=len(data_loader)):
    ...
    a = imgs['T2'].to(device)
    b = imgs['T1CE'].to(device)
    c = imgs['FLAIR'].to(device)
    d = imgs['T1'].to(device)
    ...
    G_err22= G_err2.detach_().requires_grad_(True)
    G_err22.backward(retain_graph=True)
    ...
    F_err22= F_err2.detach_().requires_grad_(True)
    F_err22.backward(retain_graph = True)
    ...
    torch.save(G,f'./model_save/{dataflie}_G_final.pt')
    # torch.save(F,f'./model_save/{dataflie}_F_final.pt')
    # torch.save(DX,f'./model_save/{dataflie}_DX_final.pt')
    # torch.save(DY,f'./model_save/{dataflie}_DY_final.pt')
