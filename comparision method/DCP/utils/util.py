...
def cal_std(logger, *arg):
    ...
    if len(arg) == 4:
        ...
        return (round(np.mean(arg[0]) * 100, 2),
                round(np.mean(arg[1]) * 100, 2),
                round(np.mean(arg[2]) * 100, 2),
                round(np.mean(arg[3]) * 100, 2))
        ...
        output = """ACC {:.2f} std {:.2f}""".format(np.mean(arg[0]) * 100, np.std(arg[0]) * 100)
        logger.info(output)

        return round(np.mean(arg[0]) * 100, 2), round(np.std(arg[0]) * 100, 2)
