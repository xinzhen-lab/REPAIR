...
def next_batch_4view(X1, X2, X3, X4, batch_size):
    # generate next batch for 4 view data
    tot = X1.shape[0]
    total = math.ceil(tot / batch_size)  # fix the last batch
    for i in range(int(total) - 1):
        start_idx = i * batch_size
        ...
        batch_x2 = X2[start_idx: end_idx, ...]
        batch_x3 = X3[start_idx: end_idx, ...]
        batch_x4 = X4[start_idx: end_idx, ...]  # 新增第四个视图
        yield (batch_x1, batch_x2, batch_x3, batch_x4, (i + 1))


def next_batch_gt_4view(X1, X2, X3, X4, gt, batch_size):
    # generate next batch for 4 view data with label
    tot = X1.shape[0]
    total = math.ceil(tot / batch_size)  # fix the last batch
    for i in range(int(total) - 1):
        start_idx = i * batch_size
        ...
        batch_x4 = X4[start_idx: end_idx, ...]  # 新增第四个视图
        gt_now = gt[start_idx: end_idx, ...]
        yield (batch_x1, batch_x2, batch_x3, batch_x4, gt_now, (i + 1))
