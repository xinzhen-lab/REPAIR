...
def get_score(representation, Y_list, acc,spe,sen, bacc):
    ...
    bacc.append(scores['kmeans']['BAcc'])
    sen.append(scores['kmeans']['SEN'])
    spe.append(scores['kmeans']['SPE'])
...
def classification_metric(y_true, y_pred, average='macro', verbose=True, decimals=4):
    ...
    sen, spe, bacc = cal_sen_spe(y_true, y_pred)
    sen = np.round(sen, decimals)
    spe=np.round(spe, decimals)
    bacc=np.round(bacc, decimals)
    # precision
    ...
    return {'accuracy': accuracy, 'precision': precision, 'recall': recall, 'f_measure': f_score,'BAcc':bacc,'SEN':sen,'SPE':spe}, confusion_matrix
