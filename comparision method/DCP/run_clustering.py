...
parser.add_argument('--missing_rate', type=float, default='0.75', help='missing rate')
...
def main():
    ...
    fold_acc, fold_sen, fold_spe,fold_bacc = [], [], [],[]
    ...
    for data_seed in range(1, args.test_time + 1):
        ...
        # Training
        acc, spe, sen, bacc = DCP_model.train_clustering(config, logger, accumulated_metrics, x1_train, x2_train, Y_list, mask,
                                             optimizer, device)
        fold_acc.append(acc)
        fold_spe.append(spe)
        fold_sen.append(sen)
    ...
    acc, spe, sen, bacc = cal_std(logger, fold_acc, fold_spe, fold_sen, fold_bacc)
    with open(f'/home/wcx/new deep learn/2022-TPAMI-DCP/result_cluster/{dataset}_{args.missing_rate}.txt', 'w') as f:
        # 写入变量的值
        f.write(f'acc: {acc}\n')
        f.write(f'Spe: {spe}\n')
        f.write(f'Sen: {sen}\n')
