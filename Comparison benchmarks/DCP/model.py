...
class DCP():
    ...
    def train_clustering(self, config, logger, accumulated_metrics, x1_train,
            ...
            if (k + 1) % config['print_num'] == 0:
                with torch.no_grad():
                    ...
                    scores = clustering.get_score([latent_fusion], Y_list, accumulated_metrics['acc'],
                                                  accumulated_metrics['spe'],
                                                  accumulated_metrics['sen'], accumulated_metrics['bacc'])
        ...
        return accumulated_metrics['acc'][-1], accumulated_metrics['spe'][-1], accumulated_metrics['sen'][-1], accumulated_metrics['bacc'][-1]
        ...
        for k in range(epochs):
            ...
            if (k + 1) % config['print_num'] == 0:
                with torch.no_grad():
                    ...
                    sen, spe, bacc = cal_sen_spe(labels_test, label_pre)
                    ...
                    accumulated_metrics['sen'].append(sen)
                    accumulated_metrics['spe'].append(spe)
                    accumulated_metrics['bacc'].append(bacc)
                    logger.info('\033[2;29m Accuracy on the test set is {:.4f}'.format(scores))
                    logger.info('\033[2;29m spe on the test set is {:.4f}'.format(spe))
                    logger.info('\033[2;29m sen on the test set is {:.4f}'.format(sen))
                    logger.info('\033[2;29m bacc on the test set is {:.4f}'.format(bacc))
                    self.autoencoder1.train(), self.autoencoder2.train()
                    self.img2txt.train(), self.txt2img.train()
