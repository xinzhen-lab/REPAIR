...
def load_data(config):
    ...
    elif data_name in ['1p19q']:

        mat = sio.loadmat(os.path.join(main_dir, 'data', '1p19q'))

        X = mat['X'][0]

        X_list.append(X[0].astype('float32'))  # View 1

        X_list.append(X[1].astype('float32'))  # View 2
        Y_list.append(np.squeeze(mat['Y']))
...
def load_multiview4_data(config):
    data_name = config['dataset']
    main_dir = sys.path[0]
    X_list = []
    Y_list = []

    if data_name in ['brats_hgg_lgg']:
        mat = sio.loadmat(os.path.join(main_dir, 'data', 'brats_hgg_lgg'))
        X = mat['X'][0]
        X_list.append(X[0].astype('float32'))  # View 1
        X_list.append(X[1].astype('float32'))  # View 2
        X_list.append(X[2].astype('float32'))  # View 3
        X_list.append(X[3].astype('float32'))  # View 4
        Y_list.append(np.squeeze(mat['Y']))
        Y_list.append(np.squeeze(mat['Y']))
        Y_list.append(np.squeeze(mat['Y']))
        Y_list.append(np.squeeze(mat['Y']))

    elif data_name in ['brats_2']:
        mat = sio.loadmat(os.path.join(main_dir, 'data', 'brats_2'))
        X = mat['X'][0]
        X_list.append(X[0].astype('float32'))  # View 1
        X_list.append(X[1].astype('float32'))  # View 2
        X_list.append(X[2].astype('float32'))  # View 3
        ...
        Y_list.append(np.squeeze(mat['Y']))
        Y_list.append(np.squeeze(mat['Y']))
    elif data_name in ['dataverse']:
        mat = sio.loadmat(os.path.join(main_dir, 'data', 'dataverse'))
        X = mat['X'][0]
        X_list.append(X[0].astype('float32'))  # View 1
        X_list.append(X[1].astype('float32'))  # View 2
        X_list.append(X[2].astype('float32'))  # View 3
        ...
        Y_list.append(np.squeeze(mat['Y']))
        Y_list.append(np.squeeze(mat['Y']))
    elif data_name in ['REMBRANDT']:
        mat = sio.loadmat(os.path.join(main_dir, 'data', 'REMBRANDT'))
        X = mat['X'][0]
        X_list.append(X[0].astype('float32'))  # View 1
        X_list.append(X[1].astype('float32'))  # View 2
        X_list.append(X[2].astype('float32'))  # View 3
        ...
        mat = sio.loadmat(os.path.join(main_dir, 'data', 'LandUse_21.mat'))
        train_x = []
        train_x.append(sparse.csr_matrix(mat['X'][0, 0]).A)  # View 1
        train_x.append(sparse.csr_matrix(mat['X'][0, 1]).A)  # View 2
        train_x.append(sparse.csr_matrix(mat['X'][0, 2]).A)  # View 3
        train_x.append(sparse.csr_matrix(mat['X'][0, 3]).A)  # View 4
        index = random.sample(range(train_x[0].shape[0]), 2100)
        for view in [1, 2, 0, 3]:  # Adjust views based on available data
            x = train_x[view][index]
            y = np.squeeze(mat['Y']).astype('int')[index]
            X_list.append(x)
            Y_list.append(y)
        ...
        mat = sio.loadmat(os.path.join(main_dir, 'data', data_name + '.mat'))
        X = mat['X'][0]
        for view in [3, 4, 5, 2] :  # Adjust views based on available data
            x = X[view]
            x = util.normalize(x).astype('float32')
            y = np.squeeze(mat['Y']).astype('int')
            X_list.append(x)
