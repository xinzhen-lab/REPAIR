...
# inp, re = load(PATH+'/train_T2_FLAIR_png/100.png')


# inp, re = load(r'C:\Users\THINKPAD\Desktop\pGAN-cGAN-master\results\pGAN_run\test_latest\images\IXI0_real_B.png')
#
#
...
def generate_nifti_mse(model, test_input, tar, image_rows, image_cols):
    ...
    ssi = ssim(target_array, prediction_array, data_range=target_array.max())
    metrics.append([patient, z, mse_sklearn, ssi])
...
checkpoint_dir = './training_checkpoints-t2-to-t1'
...
# Test
# test_data_path = r'/home/wcx/new deep learn/pix2pixRAD/MICCAI_BraTS_2019_Data_Training/HGG'
test_data_path = r'/home/wcx/new deep learn/pix2pixRAD/MICCAI_BraTS_2019_Data_Training'
patients = os.listdir(test_data_path)
image_rows = 256
image_cols = 256

save_path=r'/home/wcx/new deep learn/pix2pixRAD/3D/brats'
for patient in patients:
    ...
    PNGs_dir = 'pngs_flair'
    NIFTI_dir = 'nifti_flair'



    png_dir = os.path.join(patient_path, PNGs_dir)
    nifti_dir = os.path.join(save_path,patient)

    # 检查目录是否存在
    if not os.path.exists(png_dir):
        os.mkdir(png_dir)
    else:
        print(f"The directory '{png_dir}' already exists.")

    # 检查目录是否存在
    if not os.path.exists(nifti_dir):
        os.mkdir(nifti_dir)
    else:
        print(f"The directory '{nifti_dir}' already exists.")

    # os.mkdir(png_dir)
    # os.mkdir(nifti_dir)
    png_path = os.path.join(patient_path, png_dir)
    nifti_path = os.path.join(patient_path, nifti_dir)
    ...
    gan_flair = np.ndarray((t2_imgdata.shape[1], t2_imgdata.shape[0], t2_imgdata.shape[2],
                            3))  # the last axis is 3 because it's rgb, but we need only one
    ...
    for inp, tar in test_prediction.take(-1):
        generate_nifti_mse(generator, inp, tar, t2_imgdata.shape[1], t2_imgdata.shape[0])
        # print(f'Slice number: {z}')
        z += 1

    # Generate the nifti file from the output of pix2pix
    gan_flair_nifti = nib.Nifti1Image(np.rot90(np.fliplr(gan_flair[:, :, :, 0]), 1), affine=t2_img.affine)
    # nib.save(gan_flair_nifti, nifti_dir + '/gFLAIR.nii.gz')

    # nib.save(gan_flair_nifti, nifti_dir + '/gFLAIR.nii')
    nib.save(gan_flair_nifti, nifti_dir + f'/{patient}_t1.nii')

    ## Generate dataframe with evaluation metrics for each patient and slice
    df = pd.DataFrame(metrics, columns=["PID", "Slice", "MSE", "SSI"])
    # df.to_csv(f'BLINDED/{patient}.csv')
