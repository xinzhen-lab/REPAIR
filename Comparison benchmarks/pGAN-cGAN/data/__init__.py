...
def CreateDataset(opt):
    #data directory
    target_file=opt.dataroot+'/'+opt.phase+'/t1x_t1cey_data.mat'
    f = h5py.File(target_file,'r')
    aaa=list(f.keys())
    asd=np.array(f['data_x']).shape

    # slices = np.array(f['data_x']).shape[2] / 2
    # samples = range(np.array(f['data_y']).shape[3])

    # slices=np.array(f['data_x']).shape[3]/2#一半的层数
    samples=range(np.array(f['data_y']).shape[2])

    aa=int(opt.input_nc / 2)
    bb=int(opt.output_nc / 2)
    # import pdb;pdb.set_trace()
    # Selecting neighbouring slices based on the inputs
    if opt.which_direction=='AtoB':#模态
        ...
        # data_x = np.array(f['data_x'], dtype=np.float32)[:, :, :, :]
        # data_y = np.array(f['data_y'],dtype=np.float32)[:, :, :, :]
    else:

        data_y=np.array(f['data_y'])[:,:,:,slices-opt.input_nc/2:slices+opt.input_nc/2]
        data_x=np.array(f['data_x'])[:,:,:,slices-opt.output_nc/2:slices+opt.output_nc/2]
        # data_y = np.array(f['data_y'],dtype=np.float32)[:, :, :,:]
        # data_x = np.array(f['data_x'],dtype=np.float32)[:, :,:,:]
    #Shuffle slices in data_y for the cGAN case (incase the input data is registered)
    if opt.dataset_mode == 'unaligned_mat':  
        if opt.isTrain:
            print("Training phasepython")
