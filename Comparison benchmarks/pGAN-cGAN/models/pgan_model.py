...
class pGAN(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.vgg = VGG16(weight_path='/home/wcx/new deep learn/pGAN-cGAN-master/vgg16-397923af.pth')
        # vgg_weights=torch.load('/home/wcx/new deep learn/pGAN-cGAN-master/vgg16-397923af.pth')
        # print(vgg_weights.keys())
        # feature_weights = {k: v for k, v in vgg_weights.items() if k.startswith('features')}
        # self.vgg.load_state_dict(feature_weights)
        self.vgg.cuda()
        if self.isTrain:
...
class VGG16(torch.nn.Module):
    def __init__(self,weight_path=None):
        ...
        if weight_path:
            vgg_weights = torch.load(weight_path)
            # print(vgg_weights.keys())
            feature_weights = {k: v for k, v in vgg_weights.items() if k.startswith('features')}
            self.load_state_dict(feature_weights, strict=False)
    def forward(self, X):
