...
opts.normalized = 1;
opts.beta = 0.1; %0.1
Datafold = [Dataname,'_percent',num2str(percentDel),'.mat'];
% Datafold = [Dataname,'_',num2str(percentDel),'percent_all_missing.mat'];
load(Dataname);
load(Datafold);
truth=-(truth-2);
num_view = length(X);
...
result_CLU = ClusteringMeasure(truth, predictLabel);   

% Clu_result.AUC = result_CLU(4);
% [Clu_result.ARi,~,~,~] = RandIndex(truth, predictLabel);
Clu_result.ACC = result_CLU(1);
Clu_result.SPE=result_CLU(2);
Clu_result.SEN=result_CLU(3);
Clu_result.Purity = result_CLU(4);
Clu_result.NMI = result_CLU(5);
Clu_result.Fscore=result_CLU(6);
