...
for epoch in notebook.tqdm(range(epochs)):
  ...
  for i,imgs in notebook.tqdm(enumerate(data_loader),total=len(data_loader)):
    ...
    target = in_imgs[:, r:r + 1, :, :].clone().detach()
    in_imgs[:,r:r+1,:,:]=0
    recon = Gen(in_imgs)
    ...
    optimizer_g.step()
    loss1 = dis_loss.detach_().requires_grad_(True)
    ...
    if i%500==499 or i==0:
      ...
      plt.show()
  print('Saving Model')
  torch.save(Gen,f'./{dataflie}_generator.pth')
  torch.save(Dis,f'./{dataflie}_discriminator.pth')
