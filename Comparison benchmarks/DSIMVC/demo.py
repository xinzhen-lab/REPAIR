...
parser.add_argument('--dataset', default='brats_hgg_lgg')
...
parser.add_argument("--epochs", default=120)#120
...
if args.dataset == 'brats_hgg_lgg':
    args.initial_epochs = 50
    args.pretrain_epochs = 100
    args.iterations = 200
if args.dataset == '1p19q':
    args.initial_epochs = 50
    args.pretrain_epochs = 100
    args.iterations = 200
if args.dataset == 'dataverse':
    ...
    args.pretrain_epochs = 100
    args.iterations = 200
if args.dataset == 'REMBRANDT':
    args.initial_epochs = 50
    args.pretrain_epochs = 100
    args.iterations = 200
if args.dataset == 'brats_2':
    args.initial_epochs = 50
    args.pretrain_epochs = 100
    args.iterations = 200
...
def bi_level_train(model, criterion, optimizer, class_num, view,
    ...
    acc, nmi, pur,sen, spe, bacc = valid(model, mask)

    return acc, nmi, pur,sen, spe, bacc
...
def valid(model, mask):
    ...
    acc, nmi, pur,sen, spe, bacc = evaluate(Y, pred_vec)
    print('ACC = {:.4f} NMI = {:.4f} PUR = {:.4f}'.format(acc, nmi, pur,sen, spe, bacc))
    return acc, nmi, pur,sen, spe, bacc
...
def main():
    result_record = {"ACC": [], "NMI": [], "PUR": [],"sen": [],"spe": [], "bacc": []}
    for t in range(1, args.T+1):
        ...
        acc, nmi, pur,sen, spe, bacc = bi_level_train(model, criterion, optimizer, class_num, view, com_loader,
                 full_loader, mask, incomplete_ind)
        result_record["ACC"].append(acc)
        result_record["NMI"].append(nmi)
        result_record["PUR"].append(pur)
        result_record["sen"].append(sen)
        result_record["spe"].append(spe)
        result_record["bacc"].append(bacc)
    ...
    print("SEN (mean) = {:.4f} SEN (std) = {:.4f}".format(np.mean(result_record["sen"]), np.std(result_record["sen"])))
    print("SPE (mean) = {:.4f} SPE (std) = {:.4f}".format(np.mean(result_record["spe"]), np.std(result_record["spe"])))
    print("BACC (mean) = {:.4f} BAC (std) = {:.4f}".format(np.mean(result_record["bacc"]), np.std(result_record["bacc"])))

    filename = f"./result/{args.dataset}_{args.miss_rate}.txt"
    with open(filename, 'w') as f:
        f.write("----------------Final Results----------------\n")
        f.write("ACC (mean) = {:.4f} ACC (std) = {:.4f}\n".format(np.mean(result_record["ACC"]),
                                                                  np.std(result_record["ACC"])))
        f.write("NMI (mean) = {:.4f} NMI (std) = {:.4f}\n".format(np.mean(result_record["NMI"]),
                                                                  np.std(result_record["NMI"])))
        f.write("PUR (mean) = {:.4f} PUR (std) = {:.4f}\n".format(np.mean(result_record["PUR"]),
                                                                  np.std(result_record["PUR"])))
        f.write("SEN (mean) = {:.4f} SEN (std) = {:.4f}\n".format(np.mean(result_record["sen"]),
                                                                  np.std(result_record["sen"])))
        f.write("SPE (mean) = {:.4f} SPE (std) = {:.4f}\n".format(np.mean(result_record["spe"]),
                                                                  np.std(result_record["spe"])))
        f.write("BACC (mean) = {:.4f} BACC (std) = {:.4f}\n".format(np.mean(result_record["bacc"]),
                                                                  np.std(result_record["bacc"])))
